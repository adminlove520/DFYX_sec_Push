...
class Action:
    def __init__(self):
        ...
        self.task_list = []
    # 上传临时素材并获取media_id
    # async def upload_temp_media(self, file_path):
    ...
    async def get_access_token(self):  
        ...
        try:  
            ...
            if resp_data.get('errcode') == 0:  
                ...
                print(f'Failed to get access token: {resp_data}')  
                return None  
    ...
    async def upload_image(self, image_path):  
        ...
        try:  
            ...
            resp = await self.client.post(url, files=files, timeout=TIMEOUT)  
            resp_data = resp.json()  
            if resp_data.get('errcode') == 0:  
                return resp_data['media_id']  
            else:  
                print(f'Failed to upload image: {resp_data}')  
                return None  
        except Exception as e:  
    ...
    async def qywx(self):
        ...
        url = f'https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid={self.corpid}&corpsecret={self.corpsecret}'
        thumb_media_id = await self.upload_image('temp.jpg')  # 上传temp.jpg并获取media_id
        try:
            resp = await self.client.get(url)
